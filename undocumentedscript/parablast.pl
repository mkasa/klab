...
if(!-e "$databasename.nsq" && !-e "$databasename.psq" && !-e "$databasename.00.nsq" && !-e "$databasename.00.psq") {
    print STDERR "Database file does not exist.\n";
    exit 1;
}

# check if the specified output format is supported.
if($outputFormatType !~ /^0/ && $outputFormatType !~ /^6/ && $outputFormatType !~ /^7/) {
    print STDERR "-outfmt $outputFormatType option is not supported by paraBLAST.\n";
    print STDERR "Please contact the developers to support -outfmt $outputFormatType if you feel it appropriate.\n";
    exit 1;
}
...
my $haveToDeleteSplitFiles = 0;
{
	...
	my $cmdline = "perl splitfasta.pl --equalbase $splitqueryfilename $num_cpus";
...
EXECUTEBLASTINPARALLEL: {
	...
	push(@commandlines, "$blastpath -query $splitqueryfiles[$_] -out \$(O)$outputfile $blastoptions");
    }
    if($debug) {
...
unless($errorHasOccured) {
    ...
    if($outputFormatType =~ /^0/ || $outputFormatType =~ /^6/ || $outputFormatType =~ /^7/) {
	print STDERR "Combine strategy : blast -outfmt $outputFormatType\n";
	my $cmdline;
